...
def renameUnits(df, magnification, cameraPixelSize = 6.5, axis = 'vertical', experiment = 'TOF'):
    ...
    """inSituTrue = (df['TOF'] == 0) | (df['experiment'] == 'inSitu') # True if inSitu imaging, False if TOF
    df['xWidth_inSitu_um'] = ((axis == 'vertical')*(
                                    df['yWidth']*(cameraPixelSize/magnification)
                                                    ) +
                             (axis == 'horizontal')*(
                                    df['xWidth']*(cameraPixelSize/magnification)
                                                    ))*inSituTrue
    df['yWidth_inSitu_um'] = ((axis == 'vertical')*(
                                    df['xWidth']*(cameraPixelSize/magnification)
                                                    ) +
                             (axis == 'horizontal')*0)*inSituTrue
    df['zWidth_inSitu_um'] = ((axis == 'vertical')*0 +
                             (axis == 'horizontal')*(
                                    df['yWidth']*(cameraPixelSize/magnification)
                                                    ))*inSituTrue
    df['xWidth_TOF_um'] = ((axis == 'vertical')*(
                                    df['yWidth']*(cameraPixelSize/magnification)
                                                    ) +
                             (axis == 'horizontal')*(
                                    df['xWidth']*(cameraPixelSize/magnification)
                                                    ))*(~inSituTrue)
    df['yWidth_TOF_um'] = ((axis == 'vertical')*(
                                    df['xWidth']*(cameraPixelSize/magnification)
                                                    ) +
                             (axis == 'horizontal')*0)*(~inSituTrue)
    df['zWidth_TOF_um'] = ((axis == 'vertical')*0 +
                             (axis == 'horizontal')*(
                                    df['yWidth']*(cameraPixelSize/magnification)
                                                    ))*(~inSituTrue)"""
